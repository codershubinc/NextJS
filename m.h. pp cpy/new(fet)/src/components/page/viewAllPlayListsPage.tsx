...
import Page from '@/app/play/page';
...
function ViewAllPlayListsPage() {
    ...
    const [displaySongsPlayListId, setDisplaySongsPlayListId] = useState('')
    ...
    const displayPlayListMusicList = (id: string) => {
        console.log('playListId', id);

        if (id) {
            setDisplaySongsPlayListId(id)
            setDisplaySongs(true)
        }
    }
    if (displaySongsPlayListId && displaySongs === true) {
        return (
           <Page playId={displaySongsPlayListId} />
        )
                                ...
                                (
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 sm:w-max lg:grid-cols-3 lg:w-max w-full m-auto gap-4">
                                        {playLists.map((playlist) => (
                                            <div
                                                key={playlist.$id}
                                                onClick={() => displayPlayListMusicList(playlist.$id)}
                                            >
                                                {/* href={`/play?id=${cryptoUtil.encryptString(playlist.$id)}`} key={playlist.$id} */}
